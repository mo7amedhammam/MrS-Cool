...
public enum AlertType {
    ...
    case success(title: String? = nil,image: String? = "iconSuccess", message: String = "",buttonTitle:String = "Done",secondButtonTitle:String? = nil,mainBtnAction:(()->())? = nil,secondBtnAction:(()->())? = nil )
    case error(title: String? = nil,image:String? = "img_subtract" , message: String = "",buttonTitle:String,secondButtonTitle:String? = nil,mainBtnAction:(()->()?)? = nil,secondBtnAction:(()->()?)? = nil)
    case question(title: String? = nil,image:String? = "img_group" , message: String = "",buttonTitle:String,secondButtonTitle:String? = nil,mainBtnAction:(()->()?)? = nil,secondBtnAction:(()->()?)? = nil)
    
    func title() -> String? {
        switch self {
        case .success(title: let title,_ ,_, _, _, _, _):
            return title
        case .error(title: let title,_ ,_, _, _, _, _):
            return title
        case .question(title: let title,_ ,_, _, _,_, _):
            return title
        }
    }
    func image() -> String?{
        switch self {
        case .success(_, image:let image,_,_,_, _, _):
            return image
        case .error(_, image:let image, _, _, _, _, _):
            return image
        case .question(_, image:let image, _, _, _,_, _):
            return image
        }
    }
    ...
    func message() -> String {
        switch self {
        case .success(_,_ ,message: let message,_ ,_, _, _):
            return message
        case .error(_,_ ,message: let message,_ ,_, _, _):
            return message
            
        case .question(_,_,message:let message,_,_,_, _):
            return message
        }
    }
    ...
    var MainButtonTitle: String {
        switch self {
        case .success(_,_,_,buttonTitle:let text,_, _, _):
            return text
        case .error(_, _,_,buttonTitle:let text,_, _, _):
            return text
        case .question(_, _,_,buttonTitle:let text,_,_, _):
            return text
    ...
    var MainButtonAction: (()->()?)? {
        switch self {
        case .success(_,_,_,_,_,let action,_):
            return action
        case .error(_, _,_,_,_,let action,_):
            return action
        case .question(_,_,_,_,_,let action,_):
            return action
        }
    }
    ...
    var SecondButtonTitle: String? {
        switch self {
        case .success(_, _,_, _, secondButtonTitle:let text,_,_):
            return text
        case .error(_, _,_, _, secondButtonTitle:let text,_,_):
            return text
        case .question(_, _,_, _, secondButtonTitle:let text,_,_):
            return text
        }
    }
    var SecondButtonAction: (()->()?)? {
        switch self {
        case .success(_,_,_,_,_,_,let action):
            return action
        case .error(_, _,_,_,_,_,let action):
            return action
        case .question(_,_,_,_,_,_,let action):
            return action
        }
    }
