...
//                    error = NetworkError.apiError(code: 5, error: receivedData.message ?? "")
    ...
    func UpdateParentProfile(){
        ...
        guard let birthDateStr = birthDateStr?.ChangeDateFormat(FormatFrom: "dd  MMM  yyyy", FormatTo: "yyyy-MM-dd'T'HH:mm:ss.SSS",outputLocal: .english,inputTimeZone: appTimeZone),let genderid = selectedGender?.id, let cityid = city?.id else {return}
