    ...
    var body: some View {
        VStack(spacing:0) {
            ...
            HStack {
                ...
                Spacer()
                CustomButton(Title:currentStep == .personalData ? "Save & Next" : (currentStep == .subjectsData ? "Next":"Submit"),IsDisabled: .constant((currentStep == .subjectsData && !signupvm.isTeacherHasSubjects)||(currentStep == .documentsData && (!signupvm.isTeacherHasDocuments || signupvmdocuments.TeacherDocuments?.first?.profileStatus ?? 0 != 3)) || (currentStep == .personalData && !signupvm.isFormValid)), action: {
                    switch currentStep{
                    case .personalData:
